...
![Snapshot](https://img.shields.io/badge/snapshot-0.2.2-7C3AED?style=flat-square)
...
![Tests](https://img.shields.io/badge/tests-336%20passed%20%7C%204%20skipped-2E7D32?style=flat-square)
![Models](https://img.shields.io/badge/model%20weights-not%20included-64748B?style=flat-square)
[![Verify](https://github.com/LMSM3/LeafOS/actions/workflows/verify.yml/badge.svg)](https://github.com/LMSM3/LeafOS/actions/workflows/verify.yml)
...
> [!IMPORTANT]
> This repository is an **organic, partially working 0.2.2 snapshot**.
>
...
.\program.ps1 change-version 0.2.2 --check
```
...
bash program.sh change-version 0.2.2 --check
```
...
Open the guided, colourful constructor:

```powershell
.\leafos.ps1 pack create
```

Or create a reproducible pack from flags:
...
Preview exact models, quantizations, storage, and policy with `--dry-run`; list
the available choices with `.\leafos.ps1 pack catalog`. The constructor writes
installer-compatible artifacts only. Flower identity is passive in the
manifest; the PowerShell indicator makes it evidence-bearing only when it is
bound to a live subsystem. Runtime roles or routing are not manufactured from
presentation choices. See
[the constructor guide](markdowns/PACK_CONSTRUCTOR.md) for exact selections,
validation, accessibility controls, and the install handoff.

### Put the pack glyph in PowerShell

The optional PowerShell prompt indicator shows the active pack's declared
`identity.symbol` while an owned subsystem process is alive. An active
subsystem without a selected pack shows `🍃`; an inactive subsystem shows no
LeafOS glyph.
...
The visible glyph is typed evidence that the subsystem is active and, when
customized, that its pack identity resolved. It does not collapse separate
model-residency, provider-health, or capability-authorization gates.
The prompt path supports Windows PowerShell 5/5.1 and PowerShell 7+. Bash,
Git Bash, and WSL discover and version-probe native or Windows-interoperable
hosts before dispatch. See the generic
[shell compatibility and triage procedure](procedures/SHELL-COMPATIBILITY-AND-USER-ERROR-TRIAGE.md)
for installation, policy, path, corruption, dependency, and wrong-host diagnosis.
The same PowerShell layer now provides quiet child-process helpers to prevent
background subsystems from flashing CMD windows while keeping logs and exit
codes observable. See [PowerShell pack indicator and quiet subsystems](markdowns/POWERSHELL_PACK_INDICATOR.md).
...
The root contract validates on both PowerShell and Bash. The complete local discovery suite is green for this snapshot.
...
> [!NOTE]
> These numbers describe a complete local run on 2026-08-12. The four skips cover a historical overnight sandbox fixture that is intentionally absent from the reusable source snapshot. Live provider, model-download, and hardware qualification remain separate evidence gates.

Reproduce the local discovery run from PowerShell:
...
<details>
<summary><strong>Known limits and integration boundaries</strong></summary>

* The PowerShell installer currently bypasses the richer historical animation layer.
* The historical `sandbox/overnight-01` fixture is documented but not shipped in the reusable source snapshot.
* Deterministic tests do not substitute for live provider, model-download, or GPU qualification.
* GitHub verification checks the root contract and shell surfaces; the full hardware-aware discovery suite remains a local evidence run.
* A repository-wide license has not yet been selected.

These limits are published rather than hidden because visible history and disciplined stabilization are more useful than another private rewrite followed by an extremely confident README.
...
* [x] Bring profile, shortcut, USB, Catan2, and Bash contracts back in sync
