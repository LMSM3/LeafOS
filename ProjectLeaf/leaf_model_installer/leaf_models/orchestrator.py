...
def _has_model_like_files(path: Path) -> bool:
    if not path.is_dir():
        return False
    return any(next(path.rglob(pattern), None) is not None for pattern in ("*.gguf", "*.safetensors"))
...
def resolve_model_root(
    ...
    """Resolve one model root for plans, doctors, status, and runtime handoff."""
    if destination_root is not None:
        return destination_root.expanduser().resolve(), "explicit destination"
    ...
    return (
        (Path.home() / catalog.storage["default_relative_to_home"]).resolve(),
        "catalog per-user default",
    )


def default_model_root(catalog: Catalog = MODEL_CATALOG) -> Path:
    return resolve_model_root(catalog=catalog)[0]
...
def create_pack_plan(
    *,
    ...
    for entry in items_data:
        ...
        model = by_key[key]
        if (bool(entry.get("experimental")) or model.experimental) and not include_experimental:
            continue
        dedup = (key, quant)
    ...
    destination = resolve_model_root(destination_root, catalog)[0]
    return InstallPlan(
...
def _validate_heavy(
    ...
    confirm_heavy: Optional[Iterable[str] | str],
) -> None:
    ...
    confirmed = {confirm_heavy} if isinstance(confirm_heavy, str) else set(confirm_heavy or ())
    missing = sorted(keys - confirmed)
    if missing:
        raise OrchestrationError(
            "Heavyweight confirmation missing for: "
            + ", ".join(missing)
            + ". Repeat --confirm-heavy for every experimental model key."
        )
...
def apply_plan(
    ...
    *,
    max_workers: int = 8,
    include_experimental: bool = False,
    confirm_heavy: Optional[Iterable[str] | str] = None,
