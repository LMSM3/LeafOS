...
def _detect_doctor_root(dest: Optional[str]) -> tuple[Path, str]:
    explicit = Path(dest) if dest else None
    root, reason = resolve_model_root(explicit, MODEL_CATALOG)
    return root, reason
...
def command_verify_store(args: argparse.Namespace) -> int:
    dest = resolve_model_root(Path(args.dest) if args.dest else None)[0]
    payload = _inspect_store(destination_root=dest, verify=True)
    json_rc = _emit_json(args, payload)
    if json_rc >= 0:
        return json_rc
    ...
    invalid = len([item for item in payload["artifacts"] if not item["valid_artifact"]])
    unknown = len([item for item in payload["artifacts"] if not item["known_catalog_artifact"]])
    print(f"Invalid: {invalid}; unknown: {unknown}")
    print(f"Next: {', '.join(payload['next_actions'])}")
...
def command_inspect_store(args: argparse.Namespace) -> int:
    dest = resolve_model_root(Path(args.dest) if args.dest else None)[0]
    payload = _inspect_store(destination_root=dest, verify=False)
    json_rc = _emit_json(args, payload)
    if json_rc >= 0:
        return json_rc
    print(f"Store: {payload['destination_root']}")
    print(f"Artifacts: {len(payload['artifacts'])}")
    print("Inventory only; no verification claim was made.")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sub.add_parser("catalog", help="Show the canonical model registry")
    ...
    plan_pack.add_argument(
        "--include-experimental",
        action="store_true",
        help="Include pack entries explicitly marked experimental",
    )
    ...
    apply.add_argument(
        ...
        default=[],
        metavar="MODEL_KEY",
        help="Confirm one experimental model key; repeat for every selected key",
    )
    ...
    inspect_store = sub.add_parser("inspect-store", help="Inventory local artifacts without claiming verification")
    inspect_store.add_argument("--json", action="store_true", help="Emit structured response envelope")
    inspect_store.add_argument("--dest", default=None, help="Model storage root")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        ...
        if args.command == "inspect-store":
            return command_inspect_store(args)
