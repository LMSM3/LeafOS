...
def validate_plan_v2(plan: Any, task: dict[str, Any], work_order: dict[str, Any], run_dir: Path) -> list[str]:
    ...
    for index, step in enumerate(steps):
        ...
        if kind in {"read_files", "record_hashes"}:
            ...
            if isinstance(step.get("paths"), list):
                for value in step["paths"]:
                    if not isinstance(value, str):
                        continue
                    try:
                        candidate = policy.resolve(value)
                    except ValueError:
                        continue
                    if not candidate.is_file() and candidate not in planned_files:
                        errors.append(f"step {step_id}: input file does not exist before this step: {value}")
        elif kind == "write_file":
            if not work_order["allow_mutation"]:
                errors.append(f"step {step_id}: mutation is disabled by the work order")
            candidate: Path | None = None
            try:
                candidate = policy.resolve(str(step.get("path", "")), write=True)
            except ValueError as error:
            ...
            elif candidate is not None and candidate.suffix.lower() == ".py":
                try:
                    ast.parse(step["content"], filename=str(candidate))
                except SyntaxError as error:
                    errors.append(
                        f"step {step_id}: Python syntax error at line {error.lineno}: {error.msg}"
                    )
            elif candidate is not None and candidate.suffix.lower() == ".json":
                try:
                    json.loads(step["content"])
                except json.JSONDecodeError as error:
                    errors.append(
                        f"step {step_id}: JSON syntax error at line {error.lineno}: {error.msg}"
                    )
