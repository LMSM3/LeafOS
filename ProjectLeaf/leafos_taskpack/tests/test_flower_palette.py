...
COLOR_ENV = {
    "NO_COLOR", "FORCE_COLOR", "LEAF_COLOR", "NO_EMOJI", "LEAF_NO_EMOJI", "LEAF_GLYPHS"
}
...
class FlowerPaletteTests(unittest.TestCase):
    ...
    def test_glyph_fallback_is_independent_from_color(self) -> None:
        ...
        with self.environment(FORCE_COLOR="1", LEAF_GLYPHS="ascii"):
            self.assertEqual("LeafOS", flower_palette.glyph("🌱", "LeafOS"))
    ...
    def test_menu_actions_resolve_the_repository_from_the_module_path(self) -> None:
        ...
        self.assertTrue(
            all(str(expected_root) in action["command"] for action in actions if not action.get("disabled", False))
        )
