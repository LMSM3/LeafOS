...
class WorkOrderLoopTests(unittest.TestCase):
    ...
    def test_plan_rejects_missing_reads_and_invalid_generated_source_before_mutation(self):
        with tempfile.TemporaryDirectory() as tmp:
            base = pathlib.Path(tmp)
            target = base / "repo"
            target.mkdir()
            order = work_orders.validate_work_order(
                work_orders.load_work_order(write_work_order(base, target)), target
            )
            task = work_orders.queue_from_work_order(order, "llamacpp")[1]
            invalid = {
                "leafos_object": "agent_loop_plan",
                ...
                "task_id": task["task_id"],
                "steps": [
                    {"id": "premature", "kind": "read_files", "paths": ["main.py"]},
                    {"id": "broken", "kind": "write_file", "path": "main.py", "content": "if True print('no')\n"},
                ],
            }
            errors = work_orders.validate_plan_v2(invalid, task, order, base / "run")
            self.assertTrue(any("does not exist before this step" in error for error in errors))
            ...
            valid = {
                ...
                "task_id": task["task_id"],
                "steps": [
                    {"id": "write", "kind": "write_file", "path": "main.py", "content": "print('ok')\n"},
                    {"id": "read", "kind": "read_files", "paths": ["main.py"]},
                ],
            }
            self.assertEqual([], work_orders.validate_plan_v2(valid, task, order, base / "run"))
