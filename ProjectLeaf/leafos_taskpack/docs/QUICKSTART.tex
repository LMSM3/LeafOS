...
Create a minimal generic project:

\begin{lstlisting}
.\bin\leafctl.ps1 live C:\R\MyNewProject \
  --new --template generic --yes --budget 64
\end{lstlisting}
...
.\bin\leafctl.ps1 resident start \
  --contract-only --iterations 3
\end{lstlisting}

The current discovery suite contains 199 tests.
...
The resident implementation, contract profile, 199-test observatory, native
TUI, bounded live-provider demonstration, and real four-minute generic resident
profile pass. The real eight-minute and 64-minute soaks remain open evidence. A
profile is not complete merely because its command exists.
