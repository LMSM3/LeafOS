...
## L/M/N delivery map and X UX overlay

L, M, N, and X are navigation labels only. They do not allocate new work-order
numbers, queues, state stores, or acceptance authorities.

| Lane | Scope | Ordered owners |
|---|---|---|
| L — local trust and advanced execution | presentation repair, typed allocation, evidence integrity, real-provider preflight/inference/streaming/containment | WO-050/R1/R2, WO-051–WO-056 |
| M — coherent loop contracts | vocabulary, command parity, resources, results, demonstration freeze, outcome ledger | WO-LOOP1–WO-LOOP6 |
| N — native product proof | unified surface, one-to-one mapping, replayable demonstrator, broad scientific trial | WO-057–WO-060 |
| X — UX projection | evidence-backed colour, motion, glyphs, progress, errors, accessibility, and legal-next-action display | shell TUI across L/M/N; primarily LOOP4 and WO-057–WO-060 |

The shell TUI is the graphics implementation for X. It consumes the shared
typed read model and evidence contracts. A visible pack glyph can therefore be
evidence-backed under WO-050-R1/R2, while the TUI itself still does not invent
provider health, model residency, authorization, runtime state, or acceptance.
...
| 16A | Model-pack and installer user-path hardening proposal | [WO-053-A](WO-053-A-model-pack-installer-user-path-hardening.md) | Proposed A–G hardening scope; Day 1 not opened |
| 16B | Native preflight implementation child | [WO-053-B](WO-053-B-llamacpp-native-capability-preflight.md) | Locally verified handler/root integration; real-host result still required |
...
[W] WO-050–060 + WO-LOOP1–WO-LOOP6: reconciled L/M/N execution map with X shell-TUI UX overlay
[D] Recovery checkpoint: documentation records restored before implementation reconciliation and real-host execution
[I] PARTIAL: WO-050/R1/R2, WO-051, WO-052, and WO-053-B have local evidence; master WO-053 and later gates remain open
[V] PASS: recorded focused suites for completed local work; NOT_RUN: digest-bound real-host WO-053 preflight and all WO-054–WO-060 acceptance gates
[P] LOCAL: canonical documentation repaired in a dirty worktree; no staging, commit, provider launch, model load, or publication
[N] Reconcile the corresponding verified implementation files into canonical source, then run the opt-in WO-053 real-host preflight
