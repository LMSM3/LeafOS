# L̷E̴A̶F̸O̵S̷ 0.9.? — C̶O̸N̶T̶I̷N̸U̷A̴L̴ B̶L̶O̵O̷M̷ R̸U̶N̵T̷I̶M̸E̴
...
> **RECOVERY NOTICE:** The corrupted opening is intentional. It represents the
...
# Continual Bloom: Persistent Personalities, Native Authority, and the LeafOS Web Runtime
...
profiler, and a reusable family of workflow packages called **Flower Bloom
Templates**.

By default, LeafOS should maximize **useful hardware utilization** whenever an
...
The current transition is LeafOS `0.9.1` to `0.9.2-web`. The intended near-term
path reaches an optional two-way chat mode around `0.9.5`. The chat surface is
not the end goal. It is another controlled projection over the same durable
runtime.

## 2. The problem with ordinary multi-agent chat
...
1. What exact transcript head did each personality read?
2. Which parts of its response are observations, hypotheses, proposals, or
   validated facts?
3. What evidence supports each promoted claim?
4. Can the state be reconstructed after the process, KV cache, terminal, or
   model disappears?
5. Which component is legally permitted to mutate files or declare success?
...
### 3.1 Monday — Rescue and Analysis
...
### 3.2 Wednesday — Explore and Create
...
### 3.3 Friday — Research and Delivery
...
### 3.4 Personalities are contracts, not costumes
...
## 4. The continual transcript

### 4.1 The transcript as an event stream
...
1. A user or tool event enters the transcript.
2. Monday on the CPU and Wednesday on the GPU consume the new event
   concurrently.
3. Their proposals enter the transcript with the same read-head provenance.
4. Friday consumes both proposals and produces a synthesis.
5. Native tools execute or validate only an approved typed action.
6. Monday audits the evidence before a checkpoint is committed.
...
`runtime.quiescent`, followed by an idle state.

## 5. Proving one personality before three

The first durable implementation should contain only **Monday — Rescue and
...
`persona.json` defines identity, duties, forbidden actions, and routing
preferences. `state.json` stores the compact recoverable present: status,
cursor, transcript head, checkpoint reference, lease, blocked reason, and one
next action. `events.ndjson` contains the append-only history. `facts.ndjson`
...
## 6. Claims, evidence, and modest hallucination metrics
...
### 7.1 Inside: operational personality
...
### 7.2 Outside: deterministic visual identity
...
## 8. Pastel color and glyph mathematics
...
Optional Unicode skins may use low-order symbols such as `✾`, `✿`, `❀`, and
`❁`. The canonical log stores the semantic dimensions and seed; the renderer
selects the smallest valid presentation for the current terminal.

## 9. Brain architecture across CPU, GPU, RAM, and NVMe

The runtime is not divided into “smart chip” and “dumb chip.” It is divided by
responsibility.

### 9.1 CPU control plane
...
### 9.3 RAM as the shared warm tier
...
### 9.5 Default utilization governor
...
five-minute rolling CPU band of **80–92 percent** and a GPU band of **85–98
...
- one persistent `llama-server` model pool uses continuous batching so logical
...
The governor alternates `ACTIVE_COMPUTE` and `THERMAL_BREAK`. It pauses CPU work
at **95°C** or GPU work at **85°C**, and resumes only below **80°C CPU** and
**70°C GPU**. Missing required telemetry for ten seconds triggers quiescence and
...
## 10. Native authority and multiple interfaces
...
### 10.2 Suppressing child command windows without losing evidence

On Windows, background tools should be launched directly with
`CreateProcessW`, not through `cmd.exe /c`. The runtime uses:

- `CREATE_NO_WINDOW`;
- redirected standard output and error pipes;
- `STARTF_USESTDHANDLES`;
- `STARTF_USESHOWWINDOW` with `SW_HIDE`;
...
The same supervisor can launch `nvidia-smi`, `llama-server`, compilers, test
...
## 11. Decision and theoretical tables
...
## 12. Flower Bloom Templates

A **Flower Bloom Template** is a reusable blank infrastructure package for a
...
## 13. Profiler truth contract

### 13.1 The 0.9.1 baseline

The supplied LeafOS `0.9.1` profiler invocation used:
...
| Provider | `off` |
| Brain identity | `unknown` |
| Coder identity | `unknown` |
| Work iterations | 14 |
...
1. Missing configuration is visible.
2. Unknown identities remain unknown.
3. Provider-off runs cannot claim model throughput.
4. GPU activity is not assigned to LeafOS without ownership evidence.
5. Expected missing telemetry is an explicit unavailable state, not a pass.
6. The terminal, HTML viewer, journal, and report agree.

## 14. Numberless work orders and release gates
...
### WO — Flower Bloom Template Registry
...
1. **Operator conversation:** user and LeafOS personalities.
2. **Internal collaboration:** Monday, Wednesday, and Friday through the
   journal.
...
## 16. Planning must become infrastructure
...
## 17. Acceptance criteria for the continual bloom runtime
