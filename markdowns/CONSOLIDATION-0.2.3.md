...
| A7 | The repository's existing OpenAI endpoint/model pair should seed the new web backend | CONTRADICTED | The operator explicitly directed the backend to remain blank while awaiting its code | Preselecting `api.openai.com` / `gpt-4o` would imply a backend decision that has not been made | Keep endpoint, host, model, and allowlist blank; retain only the disabled broker boundary |
