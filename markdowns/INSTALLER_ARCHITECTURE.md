...
| Pack constructor | `ProjectLeaf/leafos_taskpack/core/pack_constructor_cli.py` | Guided or scripted `leafos pack create`; emits validated manifests but never downloads or assigns runtime roles |
...
```text
leafos pack create
    -> validates catalog keys + quantizations
    -> writes leafos.model-pack.v1 with install.items
